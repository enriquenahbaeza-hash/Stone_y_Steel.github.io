const listaMiembros = [
   {
        id: "rafa",
        gamertag: "kike2901",
        nombreReal: "Enrique",
        rango: "👑 Admin / Fundador",
        tiempoMiembro: "Desde el origen (2021)",
        tiempoJuego: "Minecraft desde 2011 (Pocket Edition) / 3 años en CITYSPOKY y Stone & Steel",
        expertoEn: "⚡ TÉCNICO, REDSTONE Y DECORATIVO",
        foto: "fotos/kike2901.jpg",
        descripcion: "Líder principal del clan y Administrador tanto del antiguo servidor CITYSPOKY como del actual Stone & Steel. Encargado de coordinar y mantener la comunidad.",
        experiencias: "Especialista en PvP, construcción de mecanismos gigantes con Redstone y diseño técnico avanzado de granjas y automatizaciones."
    }
    {
        id: "eliezer12",
        gamertag: "@YAMOSSHIJ2",
        nombreReal: "ELIEZER",
        rango: "🗡️ Miembro",
        tiempoMiembro: "Desde el origen (2021)",
        tiempoJuego: "5 años en Minecraft / 2 en CITYSPOKY",
        expertoEn: "⚡ DECORADOR Y SUPERVIVENCIA",
        foto: "fotos/ELIEZER.jpg", // <-- Ruta simplificada
        descripcion: "Jugador enfocado en  decoracion y recolección de recursos para la comunidad.",
        experiencias: "miembro destacado en la construcción de estructuras y paisajes."
    },
    {
        id: "pato",
        gamertag: "@GAMERLULI777",
        nombreReal: "JULIANO",
        rango: "👑 Admin",
        tiempoMiembro: "Desde el origen (2021)",
        tiempoJuego: "7 años en Minecraft / 2 en CITYSPOKY",
        expertoEn: "⚡ Construcción y recolectar materiales",
        foto: "fotos/juliano.jpg",
        descripcion: "Jugador enfocado en construcción a grandes alturas, experiencia básica en redstone y técnico.",
        experiencias: "Miembro destacado por participar en todos los eventos y mano izquierda del admin."
    },
    {
        id: "gaby0228",
        gamertag: "Gaby0228",
        nombreReal: "Gabriela",
        rango: "🗡️ Miembro",
        tiempoMiembro: "Desde el origen (2021)",
        tiempoJuego: "3 años (2 años en CITYSPOKY / Stone y Steel)",
        expertoEn: "⚡ DECORACIÓN Y RECOLECCIÓN",
        foto: "fotos/Gaby0228.jpg",
        descripcion: "Integrante del clan del admin (Kike) y su fiel compañera de aventuras.",
        experiencias: "Experta en diseño de interiores, paisajismo y recolección de materiales para el grupo."
    }
    {
        id: "williamcraft",
        gamertag: "Williamcraft382",
        nombreReal: "William",
        rango: "🗡️ Miembro",
        tiempoMiembro: "desde el 24/07/2026",
        tiempoJuego: "5 años en Minecraft / 2 en Stone y Steel",
        expertoEn: "🩹 CURANDERO",
        foto: "fotos/Williamcraft382.jpg",
        descripcion: "Pertenece a Stone y Steel y dentro del server al clan de gamerluli777.",
        experiencias: "Especialista en curación y apoyo estratégico para el clan."
    },
   /* 👇 COPIAS Y PEGAS ESTE BLOQUE AL FINAL Y CAMBIAS SUS DATOS
    {
        id: "pedro_craft",
        gamertag: "@Pedro_Craft",
        nombreReal: "Pedro Martínez",
        rango: "👑 Admin",
        tiempoMiembro: "Hace 1 semana",
        tiempoJuego: "1 año en Minecraft",
        expertoEn: "🏹 PvP & Arquería",
        foto: "fotos/pedro.jpg", // Nombre exacto de su foto
        descripcion: "Nuevo integrante especializado en combate.",
        experiencias: "Defensor principal en la última invasión."
    }
    hazta aqui 👍*/
];
